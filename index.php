...
set_include_path(
  ...
  PATH_SEPARATOR . './library' .            // path to Zend Framework
	PATH_SEPARATOR . './application/models' . // path to application's models directiry
	PATH_SEPARATOR . get_include_path()       // preserving the initial value
);
...
try {
	...
	echo 'Unable to start the session (' . $e->getMessage() . ')';
	exit();
}
...
Zend_Layout::startMVC();  // this makes the action views output to be used with out general layout
...
$frontController->setControllerDirectory(array(
	'default' => './application/controllers', // controllers directory
));
